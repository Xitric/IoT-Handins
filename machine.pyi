...
class ADC:
    ...
    def __init__(self, pin: Pin):
        ...
        Be very careful not to use the pins used by ESP32, especially the bootstrapping pins.
    ...
    def atten(self, atten: int):
        ...
        This allows for a wider possible input voltage range, at the cost of accuracy (the same
        number of bits now represents a wider range). The possible attenuation options are:

        ATTN_0DB - attenuation 0 dB, default (range: 0 - 1.0 V)
        ATTN_2_5DB - attenuation 2.5 dB (range 0 - 1.34 V)
        ATTN_6DB - attenuation 6 dB (range: 0 - 2.0 V)
        ATTN_11DB - attenuation 11 dB, dangerous because of maximum pin rating (range: 0 - 3.6 V)
        :param atten: int
        :return: None
        """
        ...
    ...
    def width(self, width: int):
        """This method allows for the setting of the number of bits to be utilised and returned
        during ADC reads. Possible width options are:
    ...
    def read(self) -> int:
        """
        Read the ADC value as 0-4095 across voltage range 0.0v - 1.0v. Anything above 1.0v
        will just read as 4095). Attenuation must be applied in order to increase this usable
        voltage range.
...
class PWM(object):
    """
    Pulse width modulation (PWM) is a way to get an artificial analog output on a digital pin. It
    achieves this by rapidly toggling the pin from low to high.

    PWM can be enabled on all output-enabled pins. The base frequency can range from 1Hz to 40MHz
    but there is a tradeoff; as the base frequency increases the duty resolution decreases.
    """
    ...
    def duty(self, duty: Optional[int] = None) -> Optional[int]:
        ...
        The duty cycle is how long the pin is high compared with the length of a single period
        (low plus high time). Maximum duty cycle (1023) is when the pin is high all of the time, and
        minimum (0) is when it is low all of the time, with 512 being a 50% duty. Values beyond this
        min/max will be clipped.
        :param duty:
        :return:
        """
        ...
...
def soft_reset() -> None:
    ...
    resetting the Python heap. It tries to retain the method by which the user is
    connected to the MicroPython REPL (eg serial, USB, Wifi).
    """
    ...
